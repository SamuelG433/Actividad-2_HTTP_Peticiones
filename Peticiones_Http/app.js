...
loginButton.addEventListener("click", async (e)=>{
  ...
  if (!ok){ showAlert("error", "Revisa usuario y contraseña."); shake(authForm); return; }
  ...
  try {
    ...
    shake(authForm);
    authMsg.textContent = "";
    markInvalid(inpUser); markInvalid(inpPass);
  }
});
...
registerButton.addEventListener("click", async ()=>{
  ...
  if (u.length < 3){ markInvalid(inpUser); ok=false; } else markValid(inpUser);
  if (p.length < 4){ markInvalid(inpPass); ok=false; } else markValid(inpPass);
  if (!ok){ showAlert("error", "Usuario mínimo 3 caracteres y contraseña mínimo 4."); shake(authForm); return; }
...
logoutButton.addEventListener("click", ()=>{
  ...
  clearAlert(); authMsg.textContent="";
  inpUser.classList.remove("is-valid","is-invalid");
  inpPass.classList.remove("is-valid","is-invalid");
  showAuth();
});
